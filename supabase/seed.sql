...
INSERT INTO "products" ("id", "categoryId", "name", "description", "pricePesewas", "imageUrl", "badge", "crunchLevel", "sortOrder", "isActive") VALUES
  ('brown-sugar-milk-tea', (SELECT "id" FROM "categories" WHERE "slug" = 'boba'), 'Brown Sugar Milk Tea', 'Cold black tea, brown sugar pearls, and a creamy caramel finish.', 7000, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/llyJRkMBWnkcymSN.jpg', 'Popular', 2, 3, true),
  ('strawberry-yogurt-crunch', (SELECT "id" FROM "categories" WHERE "slug" = 'yogurt'), 'Strawberry Yogurt Crunch', 'Creamy yogurt, ripe strawberries, toasted granola, and honey.', 6000, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/XUpgOdMgXrIfyvVb.jpg', 'Fresh', 2, 2, true),
  ('mango-coconut-ice-cream', (SELECT "id" FROM "categories" WHERE "slug" = 'ice-cream'), 'Mango Coconut Ice Cream', 'Golden mango ice cream with toasted coconut and mango pieces.', 5000, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/zDlwYeHmNNKzRXlO.jpg', 'Cold drop', 1, 2, true),
  ('suya-pizza', (SELECT "id" FROM "categories" WHERE "slug" = 'pizza'), 'Ghanaian Suya Pizza', 'Blistered crust, spicy suya beef, peppers, onions, and melted cheese.', 15000, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/ZWTFSdtLKmpkEvax.jpg', 'Hot', 4, 2, true),
  ('loaded-plantain-fries', (SELECT "id" FROM "categories" WHERE "slug" = 'fries'), 'Loaded Plantain Fries', 'Golden plantain fries with cheddar sauce, scallions, and smoky crumble.', 8500, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/vqqYDROgojCULLqZ.jpg', 'New', 4, 2, true),
  ('sticky-bbq-pork-bites', (SELECT "id" FROM "categories" WHERE "slug" = 'pork'), 'Sticky BBQ Pork Bites', 'Caramelized pork belly bites with barbecue glaze, sesame, and chili.', 13000, 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028829794/HkEwsafGeIcGbxGE.jpg', 'New', 5, 2, true)
ON CONFLICT ("id") DO UPDATE SET
  "categoryId" = EXCLUDED."categoryId",
  "name" = EXCLUDED."name",
  "description" = EXCLUDED."description",
