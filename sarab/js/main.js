...
function updateCartDisplay() {
    ...
    if (cartItems.length === 0) {
        ...
        container.innerHTML = '<div id="emptyCartMsg" style="text-align: center; color: #777; padding: 40px 0; font-family: \'Poppins\', sans-serif;"><i class="fas fa-shopping-basket mb-3" style="font-size: 3rem; color: #ccc;"></i><p>Your cart is empty.</p></div>';
        return;
    }
    ...
    cartItems.forEach(function(item, index) {
        ...
        var itemHtml = `
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 15px 0; border-bottom: 1px solid #eee;">
                <div style="flex: 1;">
                    <h5 style="margin: 0; font-size: 1rem; color: #333; font-weight: 600;">${item.title}</h5>
                    <div style="color: var(--primary); font-weight: 700; font-size: 0.9rem;">GH₵${item.price.toFixed(2)}</div>
                </div>
                <div style="display: flex; align-items: center; gap: 15px;">
                    <div style="display: flex; align-items: center; background: #f8f9fa; border-radius: 6px; padding: 2px 8px;">
                        <span style="font-weight: 600; font-size: 0.9rem;">x${item.qty}</span>
                    </div>
                    <button class="remove-item" data-index="${index}" style="background: none; border: none; color: #ff4d4d; cursor: pointer; padding: 5px;"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `;
    ...
    totalDisplay.textContent = 'GH₵' + totalPrice.toFixed(2);
...
document.getElementById('mpAddCart').addEventListener('click', function() {
    ...
    var priceText = document.getElementById('mpPrice').textContent.split(' ')[0]; // Gets e.g. "GH₵45.00"
    ...
    setTimeout(function() {
        ...
        // Optionally, open the cart right after adding
        // document.getElementById('cartOv').style.display = 'flex';
        // setTimeout(() => { document.getElementById('cartOv').style.opacity = '1'; }, 10);
    }, 1000);
});

// Cart Overlay Logic
document.getElementById('cartBtn').addEventListener('click', function() {
    var cartOv = document.getElementById('cartOv');
    cartOv.style.display = 'flex';
    setTimeout(() => { cartOv.style.opacity = '1'; }, 10);
    document.body.style.overflow = 'hidden';
});

document.getElementById('cartClose').addEventListener('click', closeCartPop);
document.getElementById('cartOv').addEventListener('click', function(e) {
    if (e.target === this) closeCartPop();
});

function closeCartPop() {
    var cartOv = document.getElementById('cartOv');
    cartOv.style.opacity = '0';
    setTimeout(() => {
        cartOv.style.display = 'none';
