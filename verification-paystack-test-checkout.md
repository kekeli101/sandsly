...
On 2026-08-22, the authenticated Paystack Test Mode developer settings were reloaded after the approved configuration action. The Test Webhook URL persisted as `https://sandsly.onrender.com/api/paystack/webhook`, and its server-backed `Save changes` submit control was disabled because there was no remaining unsaved change. The deployed Render health endpoint returned HTTP 200 with `{"ok":true}`, and a deliberately invalid signed-request probe returned HTTP 401 with `{"received":false}`. The probe did not create or verify a payment. The detailed signed-webhook, replay-safety, and dashboard-configuration record is maintained in [`verification-paystack-webhook.md`](./verification-paystack-webhook.md).
