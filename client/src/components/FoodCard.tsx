...
export default function FoodCard({ item, compact = false }: { item: MenuItem; compact?: boolean }) {
  ...
  const handleAdd = () => {
    addItem(item);
    toast.success(`${item.name} added`, { description: "Your bag is getting crunchy." });
  };
    ...
    <article className={`group overflow-hidden rounded-[15px] border border-[#3a3634] bg-[#292929] shadow-[4px_4px_0_#080808] transition-transform duration-200 hover:-translate-y-1 ${compact ? "flex h-full flex-col" : ""}`}>
      ...
      <div className={`flex flex-1 flex-col p-4 ${compact ? "sm:p-5" : ""}`}>
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="shrink-0 font-display text-[1.08rem] font-black text-[#ff5a1f]">{formatPrice(item.price)}</span>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-3 border-t border-[#46413e] pt-4">
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-black uppercase tracking-[0.14em] text-[#c5bcb6]">Crunch level</span>
            <span className="flex gap-1" aria-label={`${item.crunch} out of 5 crunch level`}>
              {[1, 2, 3, 4, 5].map((level) => <span key={level} className={`h-2 w-2 rounded-full ${level <= item.crunch ? "bg-[#ff5a1f]" : "bg-[#554b46]"}`} />)}
            </span>
          </div>
          <button type="button" aria-label={`Add ${item.name} to cart`} onClick={handleAdd} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-[#ff5a1f] bg-[#111111] text-[#ff5a1f] transition-all hover:bg-[#ff5a1f] hover:text-[#111111] active:scale-95">
            <Plus size={18} strokeWidth={2.7} />
          </button>
        </div>
        {!compact && <button type="button" onClick={handleAdd} className="mt-4 flex h-11 items-center justify-center gap-2 rounded-[10px] bg-[#ff5a1f] text-xs font-black uppercase tracking-[0.14em] text-[#17100d] transition-all hover:bg-[#ff6c37] active:scale-[0.98]"><Plus size={16} /> Add to cart</button>}
      </div>
    </article>
  );
}
