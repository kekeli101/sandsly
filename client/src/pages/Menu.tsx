...
export default function Menu() {
  ...
  const visibleItems = useMemo(() => catalog.products.filter((item) => item.categorySlug === selectedCategory), [catalog.products, selectedCategory]);
  ...
  return <div className="min-h-screen bg-[#0b0b0b] px-5 pb-12 pt-7 sm:px-8 md:px-12 md:pt-12"><div className="mx-auto max-w-[1100px]">
    <div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#ff5a1f]">Pick your next bite</p><h1 className="mt-2 font-display text-[clamp(2.4rem,10vw,4rem)] font-black uppercase leading-none tracking-[-0.08em] text-[#fff7f2]">Full menu</h1><p className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-[#958984]">Prices in Ghana cedis</p></div><button type="button" onClick={() => setLocation("/cart")} className="relative mb-1 hidden h-11 w-11 items-center justify-center rounded-full border border-[#48403c] text-[#f5eee9] transition-all hover:border-[#ff5a1f] hover:text-[#ff5a1f] active:scale-95 md:flex"><ShoppingCart size={18} />{itemCount > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#ff5a1f] px-1 text-[10px] font-black text-[#111111]">{itemCount}</span>}</button></div>
    <div className="mt-8 flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{categories.map((category) => <button key={category.slug} type="button" onClick={() => selectCategory(category.slug)} className={`shrink-0 rounded-full border px-5 py-3 text-[10px] font-black uppercase tracking-[0.12em] transition-all duration-200 active:scale-[0.97] ${selectedCategory === category.slug ? "border-[#ff5a1f] bg-[#ff5a1f] text-[#111111] shadow-[3px_3px_0_#000]" : "border-[#373331] bg-[#232323] text-[#b6aba5] hover:border-[#ff5a1f] hover:text-[#ff5a1f]"}`}>{category.name}</button>)}</div>
    ...
    <div className="grid gap-5 md:grid-cols-2">{visibleItems.map((item) => <FoodCard key={item.id} item={item} />)}</div>
  </div></div>;
}
